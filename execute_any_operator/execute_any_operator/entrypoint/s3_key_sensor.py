...
@click.option(
    "--bucket-name",
    default=None,
    help="Name of the S3 bucket. Only needed when `bucket_key` is not provided as a full s3:// url.",
)
@click.option(
    "--wildcard-match",
    default=False,
    help="whether the bucket_key should be interpreted as a Unix wildcard pattern.",
)
@click.option(
    "--aws-conn-id", default="aws_default", help="A reference to the s3 connection."
)
@click.option(
    "--verify",
    default=None,
    help="Whether or not to verify SSL certificates for S3 connection. By default SSL certificates are verified.",
)
@click.option(
    "--poke-interval",
    default=60,
